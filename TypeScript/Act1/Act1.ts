...
const dataHandler = async (): Promise<properties[] | undefined> => {
    try {
    //Obtenemos JSON
    const answer = await fetch('Act1.json');

    //Condicional 
    if (!answer.ok) throw new Error("No se puede cargar el JSON");
    
    //Guardamos un array de objeto response con interficie propieties
    let response:properties [] = await answer.json();
...
// Definimos que el callback espera un array de properties y no devuelve nada (void)
type RenderCallback = (data: properties[]) => void;

// 1. El Callback: Su única misión es pintar
const renderList: RenderCallback = (data: properties[]) => {
    let html: string = ``;
...
async function loadJson(e: Event, callback: RenderCallback) {
...
btnNext?.addEventListener('click', (e: Event) => loadJson(e, renderList));
...
const titleWeb = document.getElementsByTagName('h1')[0] as HTMLHeadingElement;
