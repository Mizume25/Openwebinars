...
const CardsDisplay = document.getElementById("display");
...
//Cargar JSON
const startStock = async (e) => {
    //Control de errores
    try {

        //Recibimos JSON
        const answer = await fetch("Act5.json");

        //Comprobamos
        if (!answer.ok) throw new Error('No se ha podido cargar el archivo JSON');

        //Inyectamos array de objetos
...
//FUNCION: Cargar Cards
const loadAllCards = async (callback) => {
    //Cargamos JSON
    dispensa = await callback();

    //Borramos el contenido de adentro
    CardsDisplay.innerHTML = " "
    ...
    dispensa.forEach((item) => {
        html += `<div class="card" data-id=${item.id} onclick="updateItems(${item.id});" style="text-transform: capitalize;">
    <h4>${item.nombre}</h4>
    ...
    console.log("Cards Completos Cargados")

};


//RENDERIZAR
const renderizarWeb = async (lista) => {
...
//Funcion: Renderiza item particular
const renderizarItem = async (object) => {
    ...
    //Construccion de html
    html += `<div class="card" data-id=${object.id} onclick="updateItems(${object.id});" style="text-transform: capitalize;">
    <h4>${object.nombre}</h4>
    <p><strong>Precio:</strong> ${object.precio}€</p>
    <div class="stock-container">
        <span>Stock:</span>
        <p id="stock-num-${object.id}">${object.stock}</p>
       
    </div>
     <span class="oferr">${object.oferta ? "¡EN OFERTA!" : "Precio habitual"}</span>
</div>`
    ...
    console.log("Card completo Cargados")

};

//FUNCION: Renderiza card del cliente

const renderizarCart = async (e) => {
    try {

        if (client.length == 0) return CardsDisplay.innerHTML = `
                <div class="empty-state">
                    <i class="fas fa-search"></i>
                    <p style="color:red">DEBES AGREGAR ELEMENTOS</p>
                </div>`;

        CardsDisplay.innerHTML = " "; //Borramos el contenido que haya
        ...
        client.forEach((p) => {
            html += `<div class="card" data-id=${p.id} style="background: var(--bg-client); border: 1.5px solid #61b0ff; text-transform: capitalize;">
                <h4>${p.nombre}</h4>
                <h3>Cantidad:<span id="cantidad-num-${p.id}"> ${p.cantidad}</span></h3>
                <button id="user-delete" class="btn-danger-outline" onclick="backItems(${p.id})">Restar</button>
            </div>`
        });
...
//FUNCION: Elimina la vista
const removeView = async (e) => {
    ...
    client = [];
    displayTotalUser.textContent = 0;
    displayCountItems.textContent = 0;
}
